...
**图片处理：**
- 不要使用任何外部图片 URL（unsplash、placehold.co 等）；
- 所有需要图片的位置，用 CSS 渐变、SVG 图形、或 emoji 代替，确保视觉丰富；
- 可以用 CSS background-image: linear-gradient/radial-gradient 做装饰性背景；
- 产品/人物图位置用带文字描述的占位色块（如灰色圆角矩形 + 居中文字"咖啡师照片"）。
