...
**图片生成：**
- 如果页面需要图片（如 hero 背景、产品图、装饰图），使用标记 [generate-image: 图片描述] 作为 <img> 的 src 值；
- 示例：<img src="[generate-image: 温暖的咖啡店内部环境，柔和灯光，木质桌椅]" alt="咖啡店环境">
- 系统会自动将标记替换为 AI 生成的真实图片；
- 每个页面最多使用 3 张生成图片，其余用 CSS 渐变或 emoji 装饰。
